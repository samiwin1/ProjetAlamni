...
class AdminController extends AbstractController
{
    ...
    #[Route("/admin/user/pending", name: "user_pending")]
    public function pendingUsers(\App\Repository\UserRepository $userRepository): Response
    {
        // Supposons que le champ "isPending" indique une demande d'inscription en attente
        $pendingUsers = $userRepository->findBy(['isPending' => true]);
        return $this->render('backOffice/user_pending.html.twig', [
            'users' => $pendingUsers,
        ]);
    }
...
public function deleteUser(Request $request, EntityManagerInterface $entityManager, int $id): Response
{
    ...
    if (!$currentUser || !in_array('ROLE_ADMIN', $currentUser->getRoles())) {
        ...
        /** @var \App\Entity\User $userToDelete */
    if (!$userToDelete) {
...
#[Route('/admin/user/{id}/accept', name: 'user_accept', methods: ['POST'])]
public function acceptUser(Request $request, EntityManagerInterface $entityManager, int $id): Response
{
    $user = $entityManager->getRepository(User::class)->find($id);
    if (!$user) {
        $this->addFlash('error', 'Utilisateur non trouvé.');
        return $this->redirectToRoute('user_pending');
    }
    ...
    $entityManager->flush();
    $this->addFlash('success', 'Le compte a été accepté.');
    return $this->redirectToRoute('user_pending');
}

#[Route('/admin/user/{id}/reject', name: 'user_reject', methods: ['POST'])]
public function rejectUser(Request $request, EntityManagerInterface $entityManager, int $id): Response
{
    $user = $entityManager->getRepository(User::class)->find($id);
    if (!$user) {
        $this->addFlash('error', 'Utilisateur non trouvé.');
    ...
    $entityManager->flush();
    $this->addFlash('success', 'Le compte a été rejeté.');
    return $this->redirectToRoute('user_pending');
}
